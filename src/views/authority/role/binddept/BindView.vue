...
const stretch = (tree: Array<DeptTreeNode>): Array<string> => {
...
const {
  ...
  title: `角色：${route.query.name} - 关联更多部门`,
  id: props.id,
  tableHeaders,
  singleConfirmMessage(item: SysDept) {
    return `是否绑定部门:${item.name}`;
  },
  batchConfirmMessage: "是否绑定所选部门?",
  fetchData(
    page: Page,
    id: string,
    isBind: boolean,
    condition?: SysDept
  ): Promise<IngotResponse<Page<DeptTreeNode>>> {
    return new Promise((resolve, reject) => {
      getBindDepts(id, isBind, condition)
        .then((response) => {
          resolve({
            code: response.code,
